//
// Created by Rong Zhou on 2018-08-06.
//
...
namespace ats {
    ...
    ISTask::ISTask(shared_ptr<TaskInputData> task_input)
    ...
    void ISTask::start(const json &params) {
        LOG(INFO) << "start time: " << params["start_time"];
        LOG(INFO) << "end time: " << params["end_time"];
        if (_amount_flag) {
        ...
        if (_total_time.count() <= 0 || _status == Status::STOPPED) {
            ...
            if (_total_market_vol > 0) {
                ...
                estimate_risk_value();
                auto last_remark = _remark;
                ac_frame();
                while (_stop_strategy) {
                    _risk_aversion /= 10;
                    LOG(INFO) << "adjust risk: " << _risk_aversion;
                    _stop_strategy = false;
                    _remark = last_remark;
                    ac_frame();
                    if (_stop_strategy and _risk_adjust++ >= 3) {
                        on_task_stop();
                        break;
                    }
                }
                if (_stop_strategy) {
                    on_task_stop();
                    LOG(ERROR) << "algorithm error three times";
                } else {
                    // TODO: For open test
        ...
        LOG(WARNING) << "remark is: " << _remark;
    }

    int64_t ISTask::calc_accumulative_quantity() {
        ...
        if (!_stop_strategy) {
            ...
            LOG(INFO) << "IS adjust start quantity: " << adjust_quantity << " | task id: " << _task_id;
    ...
    int64_t ISTask::get_total_quantity() {
        return min(_start_volume, _quantity.first);
    }

    string ISTask::database_verify() {
        ...
        if (!result) {
            LOG(ERROR) << "no market quote | stock has no quote: " << _ticker.first;
    ...
    void ISTask::predict_market_quote() {
    ...
    void ISTask::limit_participate_rate() {
        if (_quantity.first / _total_market_vol > 0.1) {
        ...
        _rho = min(_rho, (_start_volume / _total_market_vol * 5));
    }

    void ISTask::estimate_risk_value() {
        _risk_aversion = 1e+3 / (_start_volume * _history_price * _history_price);
        LOG(INFO) << "risk aversion value: " << _risk_aversion;
    }

    double ISTask::func_diff_hx(double param_p) {
        auto index = 1 / _phi;
        auto item_1 = _rho;
        auto max_val = max(abs(param_p) - _psi, 0.0);
        auto item_2 = pow((max_val / ((_phi + 1) * _eta)), index);
        double result = min(item_1, item_2) * (_shooting_lambda_list[0] < 0 ? -1 : 1);
        return result;
    }

    void ISTask::volume_calibration() {
        _volume_curve.clear();
        size_t i = 0;
        for (const auto vol:_remained_vol_list) {
            ...
            if ((_start_volume - vol) * _start_volume >= 0 && (i < _remained_vol_list.size() - 1)) {
                ...
                LOG(ERROR) << "order vol pieces have different signal: " << _start_volume << " | " << vol
                           << " IS: stock: " << _ticker.first << " | qty: " << _quantity.first << " | start time: "
    ...
    double ISTask::hamiltonain_system(double lambda) {
        double update_volume = _start_volume;
        auto init_lambda = lambda;
        ...
        for (size_t i = 0; i < _predict_mktvol.size(); i++) {
            auto temp_p = lambda;
            auto hx = func_diff_hx(temp_p);
            update_volume += _predict_mktvol[i] * hx;
            _remained_vol_list.push_back(update_volume);
            auto risk_term = _risk_aversion * _expect_price_volatility[i] * update_volume;
            lambda = temp_p + risk_term;
            if (init_lambda * lambda < 0) {
                lambda = 0;
            }
    ...
    void ISTask::ac_frame() {
        ...
        while ((last_volume > min(100.0, _start_volume * 0.01)) || !converge_flag) {
            ...
            if ((last_volume * _start_volume > 0) && (last_volume <= min(100.0, _start_volume * 0.01))) {
                ...
                if (_lambda_list.back() == 0 and abs(last_temp_volume - last_volume) < 0.01) {
                    if (adjust_times++ >= 10) {
                        _stop_strategy = true;
                    ...
                    low = init_lambda;
                    LOG(WARNING) << "cannot arrive target, increase init lambda value. updated lambda: "
                                 << (low + high) / 2;
                } else if (abs(last_temp_volume - last_volume) < 0.01) {
                    if (adjust_times++ >= 10) {
                        for (auto &vol : _remained_vol_list) {
                            if (vol <= 0)
                                vol = 0;
                        }
                        LOG(WARNING) << "algorithm parameter error not stopped: " << _start_volume << " | " << "stock: "
                                     << _ticker.first << " | qty: " << _quantity.first << " | start time: "
                                     << time_point_to_string(_start_time) << " | end time: "
                                     << time_point_to_string(_end_time);
                        break;
                    }
    ...
    void ISTask::save() {
        std::ofstream f_out;
        auto filename = "test_result/IS/" + to_string(_quantity.first) + "/" + _ticker.first + "_" +
        ...
        auto lambda_filename = "test_result/IS/" + to_string(_quantity.first) + "/" + _ticker.first + "_" +
                               to_string(_quantity.first) + "_" +
                               _current_date + "_" + time_point_to_string(_start_time) + "_" +
                               time_point_to_string(_end_time);
        ...
        auto shoot_lambda_filename = "test_result/IS/" + to_string(_quantity.first) + "/" + _ticker.first + "_" +
                                     to_string(_quantity.first) + "_" +
                                     _current_date + "_" + time_point_to_string(_start_time) + "_" +
                                     time_point_to_string(_end_time);
        ...
        auto shoot_volume_filename = "test_result/IS/" + to_string(_quantity.first) + "/" + _ticker.first + "_" +
                                     to_string(_quantity.first) + "_" +
                                     _current_date + "_" + time_point_to_string(_start_time) + "_" +
                                     time_point_to_string(_end_time);
        ...
        auto predict_vol_filename = "test_result/IS/" + to_string(_quantity.first) + "/" + _ticker.first + "_" +
                                    to_string(_quantity.first) + "_" +
                                    _current_date + "_" + time_point_to_string(_start_time) + "_" +
                                    time_point_to_string(_end_time);
