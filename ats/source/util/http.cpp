...
bool HTTP::request(const char *method, const char *endUrl, const char *data, Json::Object *jOutput,
...
// Get Json Object on web server.
unsigned int
HTTP::request(const char *method, const char *endUrl, const char *data, Json::Object *jOutput, Result &result,
              const char *content_type) {
...
bool
HTTP::request(const char *method, const char *endUrl, const char *data, std::string &output, const char *content_type) {
    Result result;
...
unsigned int
HTTP::request(const char *method, const char *endUrl, const char *data, std::string &output, Result &result,
              const char *content_type) {
...
unsigned int HTTP::parseMessage(std::string &output, size_t &contentLength, bool &isChunked, Result &result) {
    ...
    if (contentLength == 0 && !isChunked) {
        ...
        // Extract and interpret the header.
        // revise by shuxh char* contentLenghtPos = strstr(endStatus+2,"Content-Length:");
